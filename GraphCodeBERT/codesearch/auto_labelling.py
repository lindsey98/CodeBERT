...
for auto_label_ind in range(1000):
    ...
    **Important Notes**:
    ...
    response = client.chat.completions.create(model="gpt-4o-2024-08-06", messages=[{"role": "user", "content": promt_str}], max_tokens=500)
    ...
    for match in matches:
        try:
            ...
            comment_tokens = ast.literal_eval(match[1])  # 注释 tokens 
            code_tokens = ast.literal_eval(match[2])     # 代码 tokens 
            parsed_results.append((idx, comment_tokens, code_tokens))
        except Exception as e:
            # 遇到任何解析错误，将当前 student_idx 添加到 cannotlabeled_indices，并跳过本次循环
            print(f"Error parsing tokens for student_idx {auto_label_ind}: {e}")
            cannotlabeled_indices.append(student_idx)
            continue  # 跳到下一个循环

    # 如果 parsed_results 为空，则跳过此样本并记录
    if not parsed_results:
        print("not parsed_results", auto_label_ind)
        cannotlabeled_indices.append(student_idx)
        continue
